...
def main():
    ...
    # クエリ画像を読み込んで特徴量計算
    query_img = cv2.imread("./src/templates/cm_logo.jpg", 0)
    ...
    # マップ画像を読み込んで特徴量計算
    map_img = cv2.imread("./src/images/mesoko.jpg", 0)
    map_img = cv2.resize(
        map_img,
        (int(map_img.shape[1] * expand_map), int(map_img.shape[0] * expand_map)),
    )
    ...
    result_img = cv2.drawMatchesKnn(
        query_img, kp_query, map_img, kp_map, good[:point_num], None, flags=0
    )
    ...
    print(
        "final output score-> x: {}, y: {}, drc: {:.2f}°".format(
